...
def faceDownload(url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            savePath = 'docgen/docs/new_faces'
...
def randImgSelect(folder: str):
    folderPath = os.path.join('docgen', 'docs', folder)
    collection = os.listdir(folderPath)
    fileName = random.choice(collection)
    return fileName if folder == 'random_faces' else os.path.join('docgen', 'docs', folder, fileName)
