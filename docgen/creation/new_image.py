...
def docMaking(data):
    ...
    merger(back, Text(size=smallText, text=patronRand(data['sex']), x=row1, y=520))
    ...
    merger(back, Text(size=smallText, text=data['country'], x=row2, y=620))
    ...
    imagePath = os.path.join('docgen\docs\completed', imageName)
    ...
    if data['metadata']:
        metadataMaker(imagePath, data['country'], data['name'], data['surname'], back.size[0], back.size[1])
